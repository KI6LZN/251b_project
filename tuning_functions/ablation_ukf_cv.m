...
function [err, figureCounter] = run_line(figureCounter, noise_amp, noise_corr)
    ...
    legend("UKF - CV", "Gnd Truth", "Measured");
    hold off
...
function [ukf_cv] = generate_filters_ln(centroid, noise_flag)
    ...
    ukf_cv = trackingUKF(@constvel,@cvmeas,[centroid(1);0;centroid(2);0],...
    'StateCovariance', cov, 'ProcessNoise', cov, ...
    'MeasurementNoise', meas, 'Alpha', alpha, 'Beta', beta);
end

function [ukf_cv] = generate_filters_cir(centroid, noise_flag)
    % Create an Unscented Kalman Filter object
...
function [ukf_cv] = generate_filters_cos(centroid, noise_flag)
    % Create an Unscented Kalman Filter object
