...
% This file tests the effect that different velocity covariance parameters
% and different location covariance parameters have on the average 
% centroid error across the different filters
...
function const_vel_tune_vel_loc
    %% Prepare workspace
    close all;
    ...
    vel = 10.^(-6:1:-1);
    vel = [vel, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 10.^(2:1:3)];
    num_velo = size(vel,2);
    
    %plotting variable
    mean_vel = zeros(num_velo, 2);
    num_tracks_vel = zeros(num_velo, 2);
    
    loc = 10.^(-6:1:-1);
    loc = [loc, 0.25, 0.5, 0.7826, 1, 1.25, 1.5, 10.^(2:1:3)];
    num_loc = size(loc,2);
    
    %plotting variable
    mean_loc = zeros(num_loc, 2);
    num_tracks_loc = zeros(num_loc, 2);
    ...
    for idx = 1:2
        ...
        for iter = 1:num_velo
            ...
            for p = 1:num_frames
                ...
                [tracks, nextId] = createNewTracks_1(tracks, centroids, ...
                    bbox, unassignedDetections, nextId, vel(iter));
        ...
        %         centroid_log = [centroid_log; repmat(p,size(frame_cents,1), 1) frame_cents ];
            ...
            mean_vel(iter,idx) = mean(mean_dist(:));
            num_tracks_vel(iter,idx) = nextId-1;
        end
    end
    toc %timing
    ...
    for idx = 1:2
        ...
        for iter = 1:num_loc
            ...
            for p = 1:num_frames
                ...
                [tracks, nextId] = createNewTracks_2(tracks, centroids, ...
                    bbox, unassignedDetections, nextId, loc(iter));
        ...
        %         centroid_log = [centroid_log; repmat(p,size(frame_cents,1), 1) frame_cents ];
            ...
            mean_loc(iter,idx) = mean(mean_dist(:));
            num_tracks_loc(iter,idx) = nextId-1;
        end
    end
    toc %timing
    ...
    subplot(2,1,1);
    title_str = strcat("Average Centroid Error in Pixels, Constant Velocity, Seq. ", num2str(seq_num));
    sub_str = "Velocity Covariance";
    %title(title_str, sub_str)
    semilogx(vel, mean_vel(:,1), 'r^--', vel, mean_vel(:,2), 'bo--')
    legend("Kalman Filter", "UKF")
    ylabel("Pixels")
    ylim([min(mean_vel(:))-0.25,max(mean_vel(:))+0.25])
    xlabel("Velocity Covariance")
    set(gca, 'FontName', 'Times New Roman');

    
    subplot(2,1,2);
    semilogx(loc, mean_loc(:,1), 'r^--', loc, mean_loc(:,2), 'bo--')
    legend("Kalman Filter", "UKF")
    title_str = ""; %strcat("Average Centroid Error in Pixels, Constant Velocity");
    sub_str = "Location Covariance";
    %title(title_str, sub_str)
    ylabel("Pixels")
    ylim([min(mean_loc(:))-0.25,max(mean_loc(:))+0.25])
    xlabel("Location Covariance")
    set(gca, 'FontName', 'Times New Roman');
...
function [tracks, nextId] = createNewTracks_1(tracks, centroids, ...
    bboxes, unassignedDetections, nextId, vel)
    ...
    for i = 1:size(centroids, 1)
        ...
        kalmanFilter = configureKalmanFilter('ConstantVelocity', ...
            centroid, [0.7826, vel], [0.7826, vel], 0.7826);
...
function [tracks, nextId] = createNewTracks_2(tracks, centroids, ...
    bboxes, unassignedDetections, nextId, loc)
    centroids = centroids(unassignedDetections, :);
    bboxes = bboxes(unassignedDetections, :);
    ...
    for i = 1:size(centroids, 1)
        ...
        % Create a Kalman filter object.
        kalmanFilter = configureKalmanFilter('ConstantVelocity', ...
            centroid, [loc, 0.75], [loc, 0.75], 0.7826);
