...
function const_turn_tune_vel_loc
    %% Prepare workspace
    close all;
    ...
    %plotting variable
    mean_vel = zeros(num_velo, 1);
    num_tracks_vel = zeros(num_velo, 1);
    ...
    %plotting variable
    mean_loc = zeros(num_loc, 1);
    num_tracks_loc = zeros(num_loc, 1);
...
%     for idx = 1:2
        ...
        %         centroid_log = [centroid_log; repmat(p,size(frame_cents,1), 1) frame_cents ];
            ...
            mean_vel(iter,idx-1) = mean(mean_dist(:));
            num_tracks_vel(iter,idx-1) = nextId-1;
        end
%     end
    toc %timing
...
%     for idx = 1:2
        ...
        %         centroid_log = [centroid_log; repmat(p,size(frame_cents,1), 1) frame_cents ];
            ...
            mean_loc(iter,idx-1) = mean(mean_dist(:));
            num_tracks_loc(iter,idx-1) = nextId-1;
        end
%     end
    toc %timing
